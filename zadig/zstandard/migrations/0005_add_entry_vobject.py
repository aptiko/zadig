...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Normally this migration should have been made in the core migrations,
        # along with the addition of core.Entry.vobject. However, if this is
        # done, then in new installations South fails to add Entry.vobject for
        # the root page, because South runs all core migrations first, then
        # zstandard migrations, and the root page is created in zstandard.
        #
        # Note that "objects" below refers to the normal, not the customized
        # manager, as the frozen ORM does not have access to custom
        # managers.
        for entry in orm['core.Entry'].objects.all():
            entry.vobject = orm['core.VObject'].objects.filter(entry=entry
